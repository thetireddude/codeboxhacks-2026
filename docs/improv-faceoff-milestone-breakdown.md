...
| Scenario variety and two-player contract | In progress | Gemma 4 26B A4B generates scenarios through forced structured function calls while Gemini Flash remains the judge. The revamp uses rotating practical scene engines plus varied opening frames, reserves generic `Two people ...` openings for one in four accepted scenarios, and validates recurring weak frames before retrying. Asymmetric scenes place two specific roles differently in a shared setting without requiring immediate interaction. Validation rejects inactive third-party teaching tasks, sentient inanimate-object premises, and recurring teaching/untangling/unpacking/convincing patterns. Generation retries up to ten times; the optional semantic quality evaluator is best-effort so its own provider failure cannot cancel a valid live match. |
...
| Per-match rubric log | Complete | Final results include an evidence-grounded one-sentence overview and backend-calculated point/rating entries for every public-speaking category. On completion, durable records keyed by match ID and guest ID automatically store every skill's points/rating, overview, what went well, what to improve, and the full rubric log for future account-level progress views. |
